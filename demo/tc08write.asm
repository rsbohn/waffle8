/ TC08 DECtape unit1 write demo (logical block 8)
/ Uses the emulator’s TC08 IOT map: DTXA=6771, DTLB=6766, DTSF=6764.
/ Buffer lives at 0400-0777 (256 words); UNIT1 set to 02002 per request.

                *0200
START,          CLL CLA
                TAD BUFPTR      / AC <- buffer start
                IOT 6771        / DTXA: set transfer address
                CLA
                TAD BLOCK       / AC <- block number (8)
                TAD UNIT1       / unit select (bit10) + requested 02002
                TAD WRITE       / set write flag
                IOT 6766        / DTLB: start write
WAIT,           IOT 6764        / DTSF: skip when ready
                JMP WAIT
                HLT

UNIT1,          02000           / unit 1 selector (bit 10 set)
WRITE,          04000           / write flag
BLOCK,          0010            / logical block 8
BUFPTR,         BUFFER

/ Data buffer: 256 words at 0400-0777
                *0400
BUFFER,         0000; 0001; 0002; 0003; 0004; 0005; 0006; 0007; 0010; 0011; 0012; 0013; 0014; 0015; 0016; 0017;
